...
public class BO2 extends ConfigFile implements CustomObject {
	...
	public boolean canSpawnAt(LocalWorld world, Rotation rotation, int x, int y, int z) {
		...
		if (!spawnWater) {
			if (checkBlock.toDefaultMaterial().equals(DefaultMaterial.WATER) ||
			    checkBlock.toDefaultMaterial().equals(DefaultMaterial.STATIONARY_WATER)) {
				return false;
			}
		}

		if (!spawnLava) {
			if (checkBlock.toDefaultMaterial().equals(DefaultMaterial.LAVA) ||
			    checkBlock.toDefaultMaterial().equals(DefaultMaterial.STATIONARY_LAVA)) {
				return false;
			}
		}
	...
	protected boolean spawn(LocalWorld world, Random random, int x, int z) {
		...
		for (SpawnHandler spawnHandler : TerrainControl.SPAWN_HANDLERS) {
			if (!spawnHandler.shouldSpawn(this, rotation, world, x, y, z)) {
				return false;
			}
		}
